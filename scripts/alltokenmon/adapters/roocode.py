...
def parse_roocode(paths: Sequence[Path]) -> AdapterResult:
    return parse_cline_family("roocode", paths)


def scan(
    context: DiscoveryContext, specs: Sequence[SourceSpec]
) -> AdapterResult:
    return scan_cline_family("roocode", context, specs)
