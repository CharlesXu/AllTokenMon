...
6. Token 数按数值自动使用“亿、百万、K、Token”单位并固定保留三位小数；只有数据源提供费用时才展示费用。每条点评必须有已报告的数值、占比或比率支撑。
7. 准确区分以下状态：
   - `no_data`：未发现受支持的本地记录。
...
6. Format token counts with adaptive `亿` (100 million), `百万` (million), `K`, or `Token` units and exactly three decimal places. Show provider-reported cost only when present. Back every observation with a reported number, share, or ratio.
